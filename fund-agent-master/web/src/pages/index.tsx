...
function format_content(content: string) {
  return content.replace(/<a href="([^"]+)">([^<]+)<\/a>/g, '[$2]($1)')
    ...
    .replace(/\[来源: (?:\[graph_\d+\]\(graph_\d+\)(?:,\s*)?)+\]/g, '')
    .replace(/\[来源: (?:\[data_\d+\]\(data_\d+\)(?:,\s*)?)+\]/g, '')
    .replace(/\[来源:(?:\[doc_\d+\]\(doc_\d+\)(?:,\s*)?)+\]/g, '')
    .replace(/\[来源: (?:\[data_\d+\]\(data_\d+\)(?:,\s*)?)+\]/g, '')
    .replace(/\[来源:(?:\[data_\d+\]\(data_\d+\)(?:,\s*)?)+\]/g, '')
    .replace(/\[来源:\s+doc_\d+\]/g, '')
    .replace(/\[来源:\[doc_\d+\]\]/g, '')
    .replace(/\[来源:\[graph_\d+\]\]/g, '')
    .replace(/\[来源:\[data_\d+\]\]/g, '')
    .replace(/\[来源:(?:\[\d+\]\(\d+\)(?:,\s*)?)+\]/g, '')
    .replace(/\[来源: (?:\[\d+\]\(\d+\)(?:,\s*)?)+\]/g, '')
    .replace(/来源: (?:\d+\(\d+\)(?:,\s*)?)+\]/g, '')
    .replace(/\[来源:\[\d+\]\]/g, '')
...
const ChatBotDemo = () => {
  ...
  return (
    <div className="h-full flex flex-col min-h-0 w-full px-4 sm:px-5 lg:px-6 relative">
      {/* 顶部反馈按钮区域 */}
      <div ref={scrollContainerRef} className="absolute bg-background top-[1px] bottom-[115px] left-0 right-0 overflow-y-auto">
        ...
        <div className="bg-background fixed left-0 right-0 bottom-0 w-full px-2 sm:px-4 z-20">
