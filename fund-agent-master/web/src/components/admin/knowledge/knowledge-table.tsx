...
import { canEditKnowledge, canDeleteKnowledge, canAddKnowledge } from "@/utils/permission";
...
export function DataTable({
  ...
  const canDelete = canDeleteKnowledge(user);
  const canAdd = canAddKnowledge(user);
    ...
    },
    {
      ...
      cell: ({ row }) => (
        canEdit ? (
          ...
          <span className="truncate break-words leading-tight" title={row.original.name}>
            {row.original.name}
          </span>
        )
      ),
