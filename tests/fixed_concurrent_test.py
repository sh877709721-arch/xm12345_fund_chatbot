...
def run_concurrent_test(query_template: str, concurrent_users: int, total_requests: int, max_workers: int):
    ...
    filename = f"fixed_concurrent_test_results_{timestamp}.json"
...
def main():
    ...
    try:
        ...
        if choice == len(TEST_CONFIGS) + 1:
            ...
            concurrent_users = int(input("并发用户数 (默认2): ") or "2")
            total_requests = int(input("总请求数 (默认10): ") or "10")
            max_workers = int(input("最大工作线程数 (默认5): ") or "5")
